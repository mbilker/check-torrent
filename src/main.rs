...
fn compute_piece_hash(files: &[FileIndex]) -> Fallible<[u8; 20]> {
  ...
  for FileIndex { path, start, end, .. } in files {
    ...
    let path = path.join("/");
    let mut fd = FsFile::open(&path).with_context(|_| {
      format!("Failed to open '{}'", path)
    })?;
    if read_counter > 0 {
      fd.seek(SeekFrom::Start(read_counter as u64)).with_context(|_| {
    ...
    loop {
      ...
      let read_amount = fd.read(&mut buffer[..request_amount]).with_context(|_| {
  ...
  for f in files {
    ...
    let meta = match fs::metadata(&path) {
      ...
      Err(e) => {
        ...
        return Err(Error::from_boxed_compat(Box::new(e)).context(
          format!("Failed to get the metadata of '{}'", path)
        ).into());
      },
    };
...
fn extra_files_check(files: &[File]) -> Fallible<()> {
  ...
  let mut extra_files: Vec<_> = walker.filter_map(|entry| {
    ...
    let part_of_torrent = files.iter().any(|file| {
      let mut torrent_path = file.path.iter()
        .map(|component| OsStr::new(component));
      let mut entry_path = components.iter()
...
fn files_hash_check(files: &[File], piece_size: usize, pieces: &[u8]) -> Fallible<()> {
  ...
  piece_files
    .par_iter()
    .enumerate()
    .try_for_each_with(sender, |s, (i, (piece, files))| -> Fallible<()> {
      ...
      if computed_hash != *piece {
        bar.set_message(&format!("Hash mismatch! (#{}, digest: {:?}, piece: {:?})",
          i,
